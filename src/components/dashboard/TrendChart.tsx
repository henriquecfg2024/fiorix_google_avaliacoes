...
export function TrendChart() {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-slate-900/95 border border-slate-800 text-white rounded-xl shadow-xl p-3 text-xs space-y-1 backdrop-blur-sm">
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-all space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h3 className="text-base font-bold text-slate-900">Tendência de Avaliações</h3>
          <p className="text-xs text-slate-500 mt-0.5">
            Evolução da nota média e volume acumulado
          </p>
        </div>
