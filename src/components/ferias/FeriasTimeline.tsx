...
export function FeriasTimeline({ eventos }: FeriasTimelineProps) {
  if (!eventos || eventos.length === 0) {
    return (
      <div className="p-4 rounded-xl bg-slate-50 dark:bg-[#101019] border border-slate-200 dark:border-white/5 text-center text-xs text-slate-400 dark:text-white/40">
      ...
      {eventos.map((ev, idx) => {
        ...
        return (
          <div key={ev.id || idx} className="relative group">
            {/* Dot Indicator */}
            <div
              className={`absolute -left-6 top-1 w-3.5 h-3.5 rounded-full border-2 border-white dark:border-[#0d0d16] ${
                isConfirmado
            ...
            <div>
              <div className="flex items-center gap-2 text-xs">
                <span className="text-slate-400 dark:text-white/50 font-mono">
                ...
                <span className="font-bold text-slate-900 dark:text-white">{ev.titulo}</span>
              </div>
              <p className="text-[11px] text-slate-500 dark:text-white/60 mt-0.5">
                {ev.autorNome}
              </p>
              {ev.detalhes && (
                <p className="text-[11px] text-slate-500 dark:text-white/40 mt-1 bg-slate-50 dark:bg-[#101019] p-2 rounded border border-slate-200 dark:border-white/5 font-mono">
                  {ev.detalhes}
                </p>
              )}
            </div>
