...
export function ChatArea({
  ...
  const handleDelete = useCallback(async (msgId: string) => {
    setContextMenu(null);
    if (typeof window !== 'undefined' && !window.confirm('Deseja realmente apagar esta mensagem?')) {
      return;
    }
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.error || 'Erro ao apagar mensagem.');
      }
    } catch {
                        ...
                        <div
                          ...
                          onContextMenu={(e) => {
                            ...
                            if (!msg.isDeleted) {
                              ...
                              const x = Math.max(10, Math.min(e.clientX, (typeof window !== 'undefined' ? window.innerWidth : 800) - menuWidth - 10));
                              const y = (typeof window !== 'undefined' && e.clientY + menuHeight > window.innerHeight)
                                      ...
                                      >
                                        ...
                                        <div className="flex items-center gap-1 shrink-0">
                                          ...
                                          <button
                                            ...
                                            className="p-1 rounded-lg text-slate-400 hover:text-emerald-400 hover:bg-white/10 transition cursor-pointer"
                                          >
                          ...
                          {!msg.isDeleted && isMine && (
                            <div className="opacity-0 group-hover:opacity-100 transition self-center flex gap-0.5">
                              <button
                                ...
                                className="p-1 rounded text-slate-500 hover:text-white hover:bg-white/10 transition cursor-pointer"
                                title="Responder"
                              >
                              ...
                              <button
                                ...
                                onClick={(e) => {
                                  ...
                                  const menuWidth = 190;
                                  const menuHeight = 220;
                                  const x = Math.max(10, Math.min(rect.right - menuWidth, (typeof window !== 'undefined' ? window.innerWidth : 800) - menuWidth - 10));
                                  const y = (typeof window !== 'undefined' && rect.bottom + 4 + menuHeight > window.innerHeight)
                            ...
                            <div className="opacity-0 group-hover:opacity-100 transition self-center flex gap-0.5 order-first">
                              ...
                              <button
                                ...
                                onClick={(e) => {
                                  ...
                                  const menuWidth = 190;
                                  const menuHeight = 220;
                                  const x = Math.max(10, Math.min(rect.left, (typeof window !== 'undefined' ? window.innerWidth : 800) - menuWidth - 10));
                                  const y = (typeof window !== 'undefined' && rect.bottom + 4 + menuHeight > window.innerHeight)
                                    ? Math.max(10, rect.top - menuHeight - 4)
                                    : rect.bottom + 4;
                                  setContextMenu({ msg, x, y });
      ...
      {contextMenu && typeof document !== 'undefined' && createPortal(
        ...
        >
          ...
          {[
            ...
            ...(contextMenu.msg.remetenteId === currentUserId && (!contextMenu.msg.anexos || contextMenu.msg.anexos.length === 0) ? [
              { icon: Edit3, label: 'Editar', action: () => { setEditingId(contextMenu.msg.id); setEditText(contextMenu.msg.conteudo); setContextMenu(null); } },
            ] : []),
            ...(contextMenu.msg.remetenteId === currentUserId || isAdmin ? [
