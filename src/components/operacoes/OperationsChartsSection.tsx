...
function IngestionTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white/95 p-3.5 text-xs text-slate-800 shadow-xl dark:border-white/10 dark:bg-[#0B1020]/95 dark:text-white backdrop-blur-xl min-w-[210px]">
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/10 pb-2 mb-2">
        <span className="font-bold text-slate-800 dark:text-white/90">{label || dataPoint?.label}</span>
        <span className="text-[10px] font-mono text-slate-500 dark:text-white/50">{batches} lote(s)</span>
      </div>
      ...
      <div className="mt-2.5 pt-2 border-t border-slate-200 dark:border-white/10 flex items-center justify-between font-semibold">
        <span className="text-slate-500 dark:text-white/60">Total:</span>
        <span className="font-mono text-emerald-600 dark:text-emerald-400">{total.toLocaleString('pt-BR')} registros</span>
      </div>
    </div>
  );
}
...
function DurationTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white/95 p-3.5 text-xs text-slate-800 shadow-xl dark:border-white/10 dark:bg-[#0B1020]/95 dark:text-white backdrop-blur-xl min-w-[190px]">
      <div className="border-b border-slate-200 dark:border-white/10 pb-2 mb-2 font-bold text-slate-800 dark:text-white/90">
        {label || dataPoint?.label}
      </div>
      <div className="flex items-center justify-between text-slate-700 dark:text-white/80 py-1">
        <span>Tempo Médio:</span>
        <span className="font-mono font-bold text-blue-600 dark:text-blue-400">
          {duration > 0 ? `${(duration / 1000).toFixed(2)}s (${duration}ms)` : 'Sem lotes'}
        </span>
      </div>
      <div className="flex items-center justify-between text-slate-500 dark:text-white/60 text-[11px] pt-1 border-t border-slate-200 dark:border-white/6">
        <span>Lotes no intervalo:</span>
        <span className="font-mono text-slate-700 dark:text-white/80">{batches}</span>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0B1020]/90 p-5 sm:p-6 shadow-xl backdrop-blur-xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-white/8">
        ...
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-xl bg-slate-100 dark:bg-white/[0.04] p-1 border border-slate-200 dark:border-white/8">
            <button
              type="button"
          ...
          <button
            ...
            className="p-2 rounded-xl bg-slate-100 dark:bg-white/[0.04] hover:bg-slate-200/60 dark:hover:bg-white/8 text-slate-500 dark:text-white/60 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-white/8 transition-all disabled:opacity-40"
      ...
      {data?.summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 dark:bg-white/[0.02] dark:border-white/6 flex items-center gap-3">
            <div className="p-2 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
              <Database className="h-4 w-4" />
            </div>
            <div>
              <span className="text-[10px] uppercase font-sans text-slate-400 dark:text-white/40 block">Registros Ingeridos</span>
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 dark:bg-white/[0.02] dark:border-white/6 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] uppercase font-sans text-slate-400 dark:text-white/40 block">Lotes Processados</span>
              <span className="text-lg font-bold font-mono text-slate-900 dark:text-white">
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 dark:bg-white/[0.02] dark:border-white/6 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] uppercase font-sans text-slate-400 dark:text-white/40 block">Tempo Médio de Lote</span>
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 dark:bg-white/[0.02] dark:border-white/6 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] uppercase font-sans text-slate-400 dark:text-white/40 block">Taxa de Conclusão</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-8 p-4 sm:p-5 rounded-xl bg-slate-50/60 border border-slate-200 dark:bg-white/[0.02] dark:border-white/6 flex flex-col justify-between">
          ...
          <div className="h-[280px] w-full min-w-0" style={{ minWidth: 0, height: 280 }}>
            {loading || !isMounted ? (
              <div className="h-full w-full flex items-center justify-center text-slate-400 dark:text-white/40 text-xs">
              ...
              <div className="h-full w-full flex items-center justify-center text-slate-400 dark:text-white/30 text-xs">
        ...
        <div className="lg:col-span-4 p-4 sm:p-5 rounded-xl bg-slate-50/60 border border-slate-200 dark:bg-white/[0.02] dark:border-white/6 flex flex-col justify-between">
          <div className="mb-4">
            <h4 className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-white">
