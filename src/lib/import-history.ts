...
export async function ensureMetasImportsTable() {
  ...
  await prisma.$executeRaw`
    ALTER TABLE public.fiorix_metas_dados
      ...
      ADD COLUMN IF NOT EXISTS STATUS_META VARCHAR(100),
      ADD COLUMN IF NOT EXISTS DIAS_ATRASO INT,
      ADD COLUMN IF NOT EXISTS DIAS_CORRIDOS INT,
      ADD COLUMN IF NOT EXISTS D10_ENTREGA TIMESTAMP,
      ADD COLUMN IF NOT EXISTS TIPO VARCHAR(100),
      ADD COLUMN IF NOT EXISTS ID_NATUREZA INT,
      ADD COLUMN IF NOT EXISTS MAGNETICO VARCHAR(50),
...
export async function ensureTarefasImportsTable() {
  await prisma.$executeRaw`
    CREATE TABLE IF NOT EXISTS public.fiorix_tarefas_imports (
      id SERIAL PRIMARY KEY,
      tenant_id VARCHAR(100) NOT NULL DEFAULT '',
      import_key VARCHAR(100),
      arquivo VARCHAR(255), 
  ...
  `;
  await prisma.$executeRaw`
    CREATE UNIQUE INDEX IF NOT EXISTS fiorix_tarefas_imports_tenant_import_key
    ON public.fiorix_tarefas_imports (tenant_id, import_key);
  `;
  await prisma.$executeRaw`
    CREATE TABLE IF NOT EXISTS public.fiorix_tarefas_dados (
      id SERIAL PRIMARY KEY,
      tenant_id VARCHAR(100) NOT NULL DEFAULT '',
      PROTOCOLO INT,
      SEQ_TITULO INT,
      ...
      DT_PREVISAO TIMESTAMP,
      DIAS_PARA_PREVISAO INT,
      STATUS_PREVISAO VARCHAR(100),
      NIVEL_RISCO VARCHAR(100),
      ID_SERVICO VARCHAR(100),
      NUMERO_SERVICO VARCHAR(100),
      ITEM_SERVICO VARCHAR(100),
      DATA_SERVICO TIMESTAMP,
      VENCIMENTO_SERVICO TIMESTAMP,
      ID_TAREFA VARCHAR(100),
      TAREFA VARCHAR(255),
      DATA_CADASTRO_TAREFA TIMESTAMP,
      STATUS_TAREFA VARCHAR(100),
      DATA_ABERTURA TIMESTAMP,
      DATA_FINALIZACAO TIMESTAMP,
      SITUACAO_TAREFA VARCHAR(100),
      ID_USUARIO VARCHAR(100),
      RESPONSAVEL VARCHAR(255),
      TIPO VARCHAR(100),
      NATUREZA VARCHAR(255),
      import_id INT REFERENCES fiorix_tarefas_imports(id) ON DELETE CASCADE
    );
  `;
  await prisma.$executeRaw`
  ...
  const rows: any[] = await prisma.$queryRaw(
    Prisma.sql`
      SELECT
        ...
        SPLIT_PART(periodo, '|', 1) as "periodStart",
        SPLIT_PART(periodo, '|', 2) as "periodEnd"
      FROM public.fiorix_tarefas_imports
      WHERE tenant_id = ${tenantId}
      ORDER BY data_hora DESC;
    `
  );
