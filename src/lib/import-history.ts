...
export async function upsertProdutividadeImportRecord(input: ProdutividadeImportLogInput) {
  ...
  const importedBy = (input.importedBy || "Manual CSV").replace(/'/g, "''");
  const status = input.status.replace(/'/g, "''");
  const errorMessage =
    input.errorMessage !== undefined && input.errorMessage !== null
      ? `'${String(input.errorMessage).replace(/'/g, "''")}'`
      : "NULL";
  const periodStart = input.periodStart ? `'${input.periodStart}'::date` : "NULL";
  const periodEnd = input.periodEnd ? `'${input.periodEnd}'::date` : "NULL";
  ...
  await prisma.$executeRawUnsafe(`
    ...
    VALUES (
      '${input.importKey.replace(/'/g, "''")}',
      '${input.importKey.replace(/'/g, "''")}',
      '${fileName}',
      ${Math.max(0, Number(input.rowsCount || 0))},
      ${Math.max(0, Number(input.insertedCount || 0))},
      '${importedBy}',
      '${status}',
      ${errorMessage},
      ${periodStart},
...
export async function listProdutividadeImportLogs(): Promise<UnifiedImportRecord[]> {
  await ensureProdutividadeImportsTable();
...
export async function listProdutividadeInferredPeriods(): Promise<UnifiedImportRecord[]> {
  const rows: any[] = await prisma.$queryRawUnsafe(`
    SELECT
...
export async function listBiImports(): Promise<UnifiedImportRecord[]> {
...
export async function listMetasImportLogs(): Promise<UnifiedImportRecord[]> {
